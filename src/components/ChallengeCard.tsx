...
const ChallengeCard = ({ id, title, description, difficulty, marks, completed, locked }: ChallengeCardProps) => {
  ...
  return (
    <Card className={`overflow-hidden transition-all duration-300 hover:shadow-md ${locked ? 'opacity-70 bg-gray-100/5' : 'border-cyber-blue/30 hover:border-cyber-blue/60'}`}>
      ...
      <CardContent className="pb-2">
        <div className="h-1 w-full bg-gray-100 rounded-full overflow-hidden">
          <div className={`h-1 ${completed ? 'bg-cyber-blue' : 'bg-cyber-blue'}`} style={{ width: completed ? '100%' : '0%' }} />
        </div>
      </CardContent>
      <CardFooter>
        <Button 
